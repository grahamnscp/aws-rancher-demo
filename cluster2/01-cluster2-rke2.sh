...
Log "\__Creating cluster config-cluster2.yaml.."
cat << EOF >./local/rke2-config-cluster2.yaml
token: $RKE2_TOKEN
tls-san:
- $NODENAME
- $NODEIP
...
- ${NODE_PUBLIC_IP[2]}
- ${NODE_PUBLIC_IP[3]}
- rke-cluster2.$DOMAINNAME
EOF
ssh $SSH_OPTS ${SSH_USERNAME}@${NODEIP} "sudo mkdir -p /etc/rancher/rke2"
scp $SSH_OPTS ./local/rke2-config-cluster2.yaml ${SSH_USERNAME}@${NODEIP}:~/config.yaml
...
Log "\__Downloading kube admin-cluster2.conf locally.."
...
# Local admin.conf
mkdir -p ./local
if [ -f ./local/admin-cluster2.conf ] ; then rm ./local/admin-cluster2.conf ; fi
scp $SSH_OPTS ${SSH_USERNAME}@${NODEIP}:~/admin.conf ./local/admin-cluster2.conf
sed -i '' "s/127.0.0.1/rke-cluster2.$DOMAINNAME/g" ./local/admin-cluster2.conf
chmod 600 ./local/admin-cluster2.conf
...
function rke2nodewait
{
  ...
  while ! $READY
  do
    NRC=`kubectl --kubeconfig=./local/admin-cluster2.conf get nodes 2>&1 | egrep 'NotReady|connection refused|no such host' | wc -l`
    if [ $NRC -eq 0 ]; then
      echo -n 0
      echo
      Log " \__RKE2 cluster nodes are Ready:"
      kubectl --kubeconfig=./local/admin-cluster2.conf get nodes
      READY=true
    else
      echo -n ${NRC}.
      sleep 10
  ...
  while ! $READY
  do
    NRC=`kubectl --kubeconfig=./local/admin-cluster2.conf get pods --all-namespaces 2>&1 | egrep -v 'Running|Completed|NAMESPACE' | wc -l`
    if [ $NRC -eq 0 ]; then
      echo -n 0
      echo
...
function rke2joinnodex
{
  ...
  cat << EOF >./local/rke2-join-config-cluster2.yaml
...
EOF
  ...
  scp $SSH_OPTS ./local/rke2-join-config-cluster2.yaml ${SSH_USERNAME}@${ANODEIP}:~/config.yaml
