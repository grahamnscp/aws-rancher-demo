...
Log "\__Installing suse-observability agent on cluster3.."
...
# install observability-agent - details via obs UI adding cluster with name 'cluster3'
helm --kubeconfig=./local/admin-cluster3.conf upgrade --install suse-observability-agent suse-observability/suse-observability-agent \
     ...
     --set-string 'stackstate.cluster.name'='cluster3' \
